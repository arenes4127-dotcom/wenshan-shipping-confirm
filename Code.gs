...
const BACKEND_VERSION = '2026-08-10.29';
...
//
// M 區跟其他排性質不同：它是「總倉」——還沒編正式儲位的商品放這裡，位於文山倉的後半段，
// 座號 12~66（比一般排的 1~12 長很多，因為那是一整段區域不是單一貨架）。
// 既然在後半段，就排在地圖那些排（前半段）走完之後，讓人員一路往後走完再回包裝區，
// 不用為了幾件貨在前後段之間來回。M 佔庫存品項的 23%，是最大的一區，值得單獨處理。
//
...
const AISLE_ORDER = ['L','K','J','I','H','G','F','E','D','C','B','A','M'];
